pipeline {
        ...
        stage('build') {
            steps {
                ...
                withCredentials([string(credentialsId: 'mysql_test_db_pass', variable: 'DB_PASS')]) {
                    sh 'echo "DATABASE_URL=mysql://root:$DB_PASS@172.18.0.3:3306/simplepay" >> .env.dist'
                    sh 'mysql -h 172.18.0.3 -u root -p$DB_PASS -e "create database simplepay;"'
                }
                sh './.env.dist'
                sh 'composer install'
            }
    ...
    post {
        always {
            withCredentials([string(credentialsId: 'mysql_test_db_pass', variable: 'DB_PASS')]) {
                sh 'mysql -h 172.18.0.3 -u root -p$DB_PASS -e "drop database simplepay;"'
            }
        }
        success {
            withCredentials([string(credentialsId: 'simple_pay_ashish_token', variable: 'TOKEN')]) {
